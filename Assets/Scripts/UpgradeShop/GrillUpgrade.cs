...
public class GrillUpgrade : MonoBehaviour
{
    ...
    public void _Active()
    {
        for (int i = 0; i < itemManager.grillsDatas.Length; i++)
        {
            ...
            {
                currentLevel = itemManager.grillsDatas[i].currentLevel;
                ...
                if (currentLevel > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (j == itemManager.grillsDatas[i].currentLevel-1)
                            {
                                _Deactive();
    ...
    void _SetItemManager()
    {
        ...
        if (inList == 0)
        {
            for (int i = 0; i < itemManager.grillsDatas.Length; i++)
            {
                if (itemManager.grillsDatas[i].itemId == "")
                {
                    itemManager.grillsDatas[i].itemId = id;
                    itemManager.grillsDatas[i].currentLevel = currentLevel;
    ...
    public void _Buy()
    {
        ...
        if (currentCoin > 0)
        {
            ...
            {
                upgradePopup.GetComponent<UpgradePopup>().grillUpgrade = gameObject.GetComponent<GrillUpgrade>();
